...
function finnDatabaseId() {
  ...
  si(info.trim());
  throw new Error(
    'Fant ikke database-id. Kjør «npx wrangler d1 info dagbok» og lim id-en inn i wrangler.toml.',
  );
}
...
try {
  ...
  const kodeForfatter = await spør('Kode for skriveren:');
  const kodeLeser = await spør('Kode for leseren:');
  ...
  settHemmelighet('KODE_FORFATTER', kodeForfatter);
  settHemmelighet('KODE_LESER', kodeLeser);
  ...
  si('  2. Send «/eier ' + kodeLeser + '» til boten én gang. Da vet den at det');
