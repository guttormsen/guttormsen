 ...
 *    at det står der når leseren åpner den.
 *
 ...
 *    stedet: en dag forfatteren holder for seg selv, låses med en nøkkel utledet av
 *    forfatterens egen kode, og ligger i lageret som noe ingen andre kan lese –
...
/**
 * Koden forfatterens blir til en nøkkel. Saltet er fast: uten en server finnes det
...
async function lagNokkel(kode) {
  ...
  return crypto.subtle.deriveKey(
    { name: 'PBKDF2', salt: ENC.encode('dagbok/v1'), iterations: 250000, hash: 'SHA-256' },
    grunn,
...
async function loggInn({ hvem: rolle, kode }) {
  ...
  if (rolle === FORFATTER) {
    ...
    if (!snap.exists) {
      // Første gang: koden forfatteren taster nå, er koden.
...
/** Setter en dag sammen igjen: det åpne, pluss det forfatteren kan låse opp. */
...
/** Dagen slik leseren får se den: en privat dag har ingen innside. */
...
async function tilstand() {
  ...
  const felles = {
    ...
    behov: BEHOV,
    delt: false,
    // Her er koden forfatterens også nøkkelen til det forfatteren holder for seg selv. Da
    // kan ingen andre sette en ny – da ville innholdet blitt uleselig.
    kan_bytte_egen: hvem === FORFATTER,
    kan_bytte_andres: false,
  };
  ...
  return {
    ...felles,
    hvem: LESER,
    idag: forLeseren(alle.find((d) => d.dato === dato) ?? null),
...
/** Alle gode ting forfatteren kan se. Leseren ser bare dem fra delte dager. */
async function lapper(alle) {
  const ut = [];
  for (const rad of alle) {
    if (hvem !== FORFATTER && rad.privat) continue;
    const dag = hvem === FORFATTER ? await lesDag(rad) : rad;
    for (const g of dag?.gode_ting ?? []) {
      if (g?.tekst) ut.push({ dato: rad.dato, tekst: g.tekst, merket: Boolean(g.merket), privat: Boolean(rad.privat) });
    }
  }
  return ut;
}
...
async function arkiv(sporring) {
  ...
  const bareMerket = p.get('merket') === 'ja';
  ...
  const treff = (await lapper(alle)).filter((g) =>
    (!bareMerket || g.merket) && (!sok || g.tekst.toLowerCase().includes(sok)));
...
export async function api(full, { metode = 'GET', kropp } = {}) {
  ...
  if (sti === '/aarsbok') {
    ...
    const synlige = erForfatter ? alle : alle.filter((r) => !r.privat);
    const maneder = new Map();
    let antall = 0;
    for (const rad of synlige) {
      const dag = erForfatter ? await lesDag(rad) : rad;
      const md = rad.dato.slice(0, 7);
      if (!maneder.has(md)) maneder.set(md, []);
      for (const g of dag?.gode_ting ?? []) {
        if (!g?.tekst) continue;
        antall += 1;
        maneder.get(md).push({ dato: rad.dato, tekst: g.tekst, merket: Boolean(g.merket) });
      }
    }
    return {
      ar,
  ...
  if (sti === '/dag' && metode === 'GET') {
    ...
    return { dag: erForfatter ? await lesDag(rad) : forLeseren(rad) };
  }

  if (sti === '/dag' && metode === 'POST') {
    if (!erForfatter) throw new Error('Bare forfatteren skriver kveldsrunden.');
    return lagreDag(kropp ?? {});
  }

  /**
   * Bytt kode. Bare forfatterens egen, og bare av forfatteren: koden er nøkkelen til det
  ...
  if (sti === '/kode' && metode === 'POST') {
    if (!erForfatter || kropp?.hvem !== FORFATTER) throw new Error('Her kan bare forfatteren bytte sin egen kode.');
    ...
    nokkel = nyNokkel;
    return { ok: true, hvem: FORFATTER, lastOpp: åpnet.length };
  }
  ...
  if (sti === '/brev' && metode === 'POST') {
    if (erForfatter) throw new Error('Bare leseren legger inn brev.');
  ...
  if (åpne && metode === 'POST') {
    if (!erForfatter) throw new Error('Ikke ditt å åpne.');
