 ...
 *
 * 1. **Forfatteren bestemmer.** Er dagen merket privat, går ingenting ut – uansett
 *    hvor lav den er. Fritekst deles bare hvis forfatteren har huket det av.
 * 2. **Varsler bare når det betyr noe.** Får leseren pling hver kveld, slutter
 *    man å se etter. Kommer det pling bare når noe er tungt, blir appen et
 *    alarmanlegg, og da slutter forfatteren å skrive. Derfor går både de tunge og de
...
/** Hva det kan bes om. `hast` avgjør om varselet kommer med lyd. */
export const BEHOV = {
 ...
 * vite at det er tungt, og så skjer det ingenting. Ett trykk her, og forfatteren ser
...
export function ryddDag(rå = {}, delt = false) {
  ...
  // ville utløst varsel om en tung dag ingen har sagt at de hadde.
...
export function varslerForDag({ dag, igår = null, sendt = [] }) {
  ...
  if (dag.humor <= 2) {
    // Har forfatteren allerede bedt om å bli ringt, er «tung dag» bare støy oppå det.
  ...
  // en stille oppsummering – ellers ville bare ytterpunktene nådd leseren, og
...
const OVERSKRIFT = {
  rop: (dag) => `📞 Forfatteren trenger deg nå – «${BEHOV[dag.behov].etikett.toLowerCase()}»`,
  tung: () => '🫂 En tung dag',
  monster: () => '🫂 Andre tunge dagen på rad',
  god: () => '✨ En skikkelig god dag',
  dagen: (dag) => `🫙 Dagen er ført – ${HUMOR[dag.humor]}`,
  morgen: () => '☀️ God morgen. I går var tung',
};
...
export function melding(slag, dag) {
  ...
  if (dag.behov && slag !== 'rop') {
    linjer.push(`Forfatteren trenger: ${BEHOV[dag.behov].etikett.toLowerCase()}`);
  }

  if (dag.del_tungt && dag.tungt) {
    linjer.push('', `Forfatteren skrev: «${dag.tungt}»`);
  }
 ...
 *
 * Et varsel klokka 22:40 er lite verdt – da sover leseren. Dette er den samme
...
/** Tallene for en periode, og ordene om den beste og den tyngste dagen. */
export function sammendragstekst(d, behovTabell = BEHOV) {
  ...
  linjer.push(`${d.antall_gode_ting} gode ting${d.merket ? `, ${d.merket} merket` : ''}`);
  ...
  if (d.behov.length) {
    linjer.push('', 'Forfatteren har bedt om:');
...
/**
 * Hvor langt lista er kommet, og de siste svarene.
 *
 * Appen deler lista i kategorier, så den gjør boten også. Ellers står det
 * bare ett tall her, og det sier ingenting om hva forfatteren har lyst til å tenke på.
 */
export function sporsmalstekst({ ferdig, igjen, svarte, perKategori = null }) {
  const linjer = [`📝 Svart på ${ferdig} av ${ferdig + igjen} spørsmål.`];
  if (perKategori) {
    linjer.push('');
...
/**
 * Forfatterens egen liste. Ikke kveldsrunden – dette er spørsmål man svarer på når
 ...
 * annet spørsmål enn det forfatteren svarte på.
 */
export const LISTA = [
  ...
  { k: 13, kat: 'nare', t: 'Hvem er du mest deg selv sammen med?' },
  { k: 14, kat: 'nare', t: 'Hvem har betydd mest for deg det siste året?' },
  { k: 15, kat: 'nare', t: 'Hva gjør du bare sammen med noen bestemte?' },
  { k: 16, kat: 'nare', t: 'Hva legger du merke til hos folk som de ikke vet?' },
  { k: 17, kat: 'nare', t: 'Hvem beundrer du for noe de er gode til?' },
  { k: 18, kat: 'nare', t: 'Hvem savner du når det har gått en stund?' },
  { k: 19, kat: 'nare', t: 'Hvilken vane har du plukket opp fra noen andre?' },
  { k: 20, kat: 'nare', t: 'Hva vil du bli flinkere til med folk rundt deg?' },
  { k: 21, kat: 'nare', t: 'Hva er den beste samtalen du har hatt?' },
  { k: 22, kat: 'nare', t: 'Hva skulle du sagt oftere, og til hvem?' },
  { k: 23, kat: 'nare', t: 'Hvem skulle du ønske du tok kontakt med?' },
  { k: 24, kat: 'nare', t: 'Hva slags gammel person tror du at du blir?' },
...
export const milepæl = (n) => [
  '🎉 Milepæl!',
  '',
  `Det er skrevet ${n} gode ting i glasset.`,
  '',
...
export const MENY = [
  ...
  [['🔑 Logg meg inn', 'meny:logginn'], ['🔗 Lenke til forfatteren', 'meny:lenkeforfatter']],
];
...
export const MENYTEKST = [
  ...
  '  /si <tekst> – melding til forfatteren',
  ...
  '  /sporsmal – hvor langt lista er kommet',
  '  /kode <ny kode> – ny kode for forfatteren',
...
export const HENDELSER = {
  innlogging: () => '🔓 Forfatteren logget inn i appen.',
  begynt: () => '✍️ Kveldsrunden er i gang.',
  lest: () => '👀 Meldingene dine er lest.',
  deling: (på) => (på
    ? '🔓 Delt modus er slått på. Du ser alt som føres nå.'
    : '🔒 Delt modus er slått av. Nå velges det dag for dag.'),
  kode: () => '🔑 Forfatteren byttet koden sin.',
  huket: (tekst) => `✓ Huket av: «${tekst}»`,
  reaksjon: (tegn, tekst) => `${tegn} Reaksjon på «${tekst}»`,
  svarte: (sporsmal, svar) => `📝 Svar på «${sporsmal}»\n\n«${svar}»`,
  etterslep: (dato, humor) => [
    `🗓 Ført i etterkant: ${dato} i etterkant.`,
    '',
    `${humor} av 5 – ${HUMOR[humor]}`,
    '',
    'Varsler for gamle dager sier lite om hvordan det står til nå, så dette er alt.',
  ].join('\n'),
  angret: (tekst) => `○ Haken tatt bort på «${tekst}»`,
};

/* ---------- svar på spørsmål fra Telegram ---------- */

/** Dagen, slik den spørres etter. */
export function dagsrapport(dag, dato) {
  ...
  if (dag.behov) linjer.push(`Forfatteren trenger: ${BEHOV[dag.behov].etikett.toLowerCase()}`);
  ...
  if (dag.holdt_gode) linjer.push('', 'De gode tingene er ikke delt.');
  if (dag.tungt) linjer.push('', `Forfatteren skrev: «${dag.tungt}»`);
...
export function onskeliste(onsker) {
  if (!onsker.length) return '✨ Ønskelista er tom. Legg til med «/onske <noe som skal gjøres>».';
...
export function stilleDager(antall) {
  return [
    `🫙 Det er ${antall} dager siden sist det ble skrevet noe her.`,
    '',
    'Trenger ikke bety noe. Men nå vet du det.',
  ].join('\n');
}

/** Når forfatteren åpner et brev, får leseren vite det. */
export function brevÅpnet(laget) {
  return `💌 Brevet du la inn ble åpnet ${laget}.`;
}

/**
 * En melding. Den går alltid gjennom – en melding er noe man
 * uttrykkelig har sendt til leseren, ikke noe appen har tolket seg fram til.
 */
...
/**
 * Forfatteren har åpnet appen. Sendes stille, og høyst én gang i timen – ellers blir
 * det et pip hver gang forfatteren bytter fane.
 */
export function erAktiv(klokkeslett) {
  return `👋 Noen er inne i appen nå (${klokkeslett}).`;
}

/** Noe nytt på ønskelista. Går stille; det haster aldri. */
export function nyttOnske(tekst) {
  return `✨ Lagt til på ønskelista: «${tekst}»`;
}
