 ...
 * ikke en feil man har gjort, og det hjelper ingen å få «Ikke logget inn»
 * som en liten svart boble på en skjerm som ellers ser levende ut – forfatteren skal
...
/** Hvilken milepæl som er neste, og hvor langt det er dit. */
...
/**
 * Krukka, med så mange ting oppi som forfatteren har skrevet.
 *
...
function visLoggInn(feilmelding) {
  ...
  const knapper = ['forfatter', 'leser'].map((navn) =>
    el('button', {
      type: 'button',
      'aria-pressed': 'false',
      'data-navn': navn,
      text: navn === 'forfatter' ? 'Skriver' : 'Leser',
      onclick: () => {
        hvem = navn;
 ...
 *
 * Forfatteren skriver den på sofaen, med dårlig dekning og en telefon som gjerne
 * kaster fanen når forfatteren bytter app. Da skal ikke tre gode ting være borte.
 * Utkastet ligger her til dagen faktisk er lagret, og bare da.
 *
 * Det ligger lokalt på forfatterens egen telefon og når hverken serveren eller leseren.
 */
const UTKASTNØKKEL = (dato) => `dagbok:utkast:${dato}`;
...
/** Har forfatteren skrevet noe i det hele tatt? Et tomt utkast er ikke verdt å tilby. */
...
async function kveldsrunden(t, ferdig, dato = t.dato) {
  ...
  const utkast = {
    ...
    merkede: [0, 1, 2].map((i) => Boolean(fra?.gode_ting?.[i]?.merket)),
    tungt: fra?.tungt ?? '',
    svar: fra?.svar ?? '',
  ...
  // Slik dagen står på serveren. Alt som måles mot denne, er det forfatteren har
  // skrevet siden – og bare det er verdt å kalle et ulagret utkast.
  const utgangspunkt = JSON.stringify(utkast);

  // Lot forfatteren appen ligge midt i runden, tar vi opp tråden der den lå.
  ...
  fanerad.replaceChildren();
  // Si fra at forfatteren er i gang. Det er den ene hendelsen serveren ikke kan se selv.
  ...
  const ramme = (tittel, undertittel, innhold, { videre = 'Videre', kanVidere = true, siste = false } = {}) => {
    ...
    // så det som står her er det forfatteren faktisk har skrevet.
    taVare();
  ...
  const stegHumor = () => {
    ...
    ramme(
      ...
      gjenopptatt
        ...
        : (gammel ? 'Du fyller ut en dag som har vært. Leseren får én stille beskjed om det, ikke et varsel.' : datoOrd(dato)),
  ...
  const stegGode = () => {
    const felt = [0, 1, 2].map((i) => {
      ...
      return el('div', { style: i < 2 ? 'margin-bottom:1.1rem' : null }, [
        ...
        el('label', { class: 'hake' }, [
          el('input', {
            type: 'checkbox', checked: utkast.merkede[i],
            onchange: (e) => { utkast.merkede[i] = e.target.checked; merkEndring(); },
          }),
          el('span', { text: 'Merk denne' }),
        ]),
      ]);
    });
  ...
  const stegBehov = () => {
    ...
    ramme('Hva trenger du?', 'Velg én, eller ingen.', [
      ...
      t.delt && el('p', {
        ...
        text: 'Denne appen står i delt modus: alt du fører her, er delt.',
      }),
      el('h3', { style: 'margin-top:1.4rem', text: 'Dette går videre' }),
  ...
  const lagre = async () => {
    try {
      const svar = await api('/dag', {
        ...
        kropp: {
          ...
          gode_ting: utkast.gode
            .map((tekst, i) => ({ tekst, merket: utkast.merkede[i] }))
      ...
      glemUtkast(dato);
      si(svar.sendt.length ? 'Lagret. Beskjed sendt.' : 'Lagret.');
      ut();
    } catch (e) {
      // Utkastet blir liggende. Sies det fra om det, slipper man å lure på
...
function forhåndsvisning(u, behov) {
  if (u.privat) {
    return el('div', { class: 'sendes' }, [
      ...
      '\n\nDet vises at du førte noe denne datoen – ikke hva.',
    ].join(''));
  }
  ...
  linjer.push('', VARSLER
    ? (lyd ? '→ Leseren får varsel på telefonen nå.' : '→ Leseren får en stille beskjed nå.')
    : '→ Det står her når leseren åpner appen.');
...
const godeTingListe = (gode) =>
  el('ul', { class: 'liste' }, gode.map((g) =>
    el('li', {}, [g.merket && el('span', { class: 'merke', text: '✦' }), g.tekst])));
...
function dagsark(dag, { egen }) {
  ...
  return el('div', { class: 'ark', style: 'margin-top:1.1rem' }, [
    ...
    el('p', { class: 'humor-linje' }, [
      ...
      `${humorOrd(dag.humor)}${dag.privat ? ' · bare for forfatteren' : ''}`,
    ]),
    ...
    dag.gode_ting?.length
      ? el('ul', { class: 'gode' }, dag.gode_ting.map((g) =>
        el('li', { 'data-merket': g.merket ? 'ja' : null }, [g.merket && '✦', g.tekst])))
      : null,
    dag.holdt_gode && el('p', { class: 'hjelp', text: 'De gode tingene er ikke delt.' }),
...
function uleste(t) {
  const fra = t.hvem === 'forfatter' ? 'leser' : 'forfatter';
...
function faneIdag(t) {
  const erForfatter = t.hvem === 'forfatter';
  const dag = t.idag;
  ...
  const hilsen = nyMelding && el('div', { class: 'kort løftet' }, [
    el('p', { class: 'stempel', text: 'Ny melding' }),
    el('p', {
    ...
    el('button', {
      ...
      onclick: () => { fane = 'meldinger'; visApp(t); },
      text: 'Svar →',
    }),
  ]);
        ...
        el('div', { class: 'glass-rad' }, [
          ...
          el('div', {}, [
            ...
            t.delt && el('p', {
              ...
              text: 'Delt modus: alt du fører her, er delt.',
            }),
          ]),
        ]),
        el('button', {
        ...
        onclick: async () => {
          try {
            ...
            tegn(el('div', { class: 'kort' }, [
              el('p', { class: 'stempel', text: 'Fra leseren' }),
  ...
  tegn(
    ...
    el('div', { class: dag && !dag.privat && dag.humor <= 2 ? 'kort løftet' : 'kort' }, dag
      ? dag.privat
        ? [
          ...
          el('p', { class: 'svak liten', style: 'margin:0', text: 'Forfatteren har ført dagen, men valgt å ikke dele den.' }),
        ]
        : [
          el('p', { class: 'stempel', text: `I dag · ført ${klokkeslett(dag.skrevet_kl)}` }),
          el('h2', { style: 'margin-top:.45rem', text: `${humorFjes(dag.humor)} ${humorOrd(dag.humor)}` }),
          dag.behov && el('p', { style: 'margin-top:.6rem' }, [
            el('strong', { text: 'Forfatteren trenger: ' }),
          ...
          dag.holdt_gode && el('p', { class: 'liten svak', text: 'De gode tingene er ikke delt.' }),
          dag.tungt && el('p', { class: 'sendes', style: 'margin-top:.7rem', text: dag.tungt }),
          dag.holdt_tungt && el('p', { class: 'liten svak', style: 'margin-top:.7rem', text: 'Noe var tungt. Forfatteren valgte å ikke dele det.' }),
    ...
    el('div', { class: 'seksjon' }, [
      el('h2', { text: 'Om oss' }),
      t.merket.length
        ? el('ul', { class: 'liste' }, t.merket.slice(0, 5).map((g) =>
    ...
    t.onsker.length
      ? el('div', { class: 'onsker' }, t.onsker.map((o) =>
        el('div', { class: 'onske', 'data-gjort': o.gjort_kl ? 'ja' : null }, [
          ...
          el('p', {}, [
            o.tekst,
            el('span', { class: 'hvem', text: ` · ${o.laget_av === t.hvem ? 'du' : (o.laget_av === 'forfatter' ? 'skriver' : 'leser')}` }),
          ]),
          el('button', {
      ...
      : el('p', { class: 'hjelp', text: 'Tom foreløpig. Hva har du lyst til?' }),
...
/** Brevene leseren legger inn på forhånd. */
function brevSeksjon(t) {
  const felt = el('textarea', { id: 'brev', placeholder: 'Noe forfatteren skal lese når det er vanskelig …' });
  const uåpnet = t.brev.filter((b) => !b.apnet_kl).length;
  return el('div', { class: 'seksjon' }, [
    el('h2', { text: 'Brev til en dårlig dag' }),
    el('p', {
      class: 'hjelp',
      text: `${uåpnet === 0 ? 'Ingen ligger klare' : uåpnet === 1 ? 'Ett ligger klart' : `${uåpnet} ligger klare`}. Forfatteren får tilbud om ett når dagen er tung.`,
    }),
    felt,
    el('button', {
...
function faneKalender(t) {
  ...
  const ruter = [
    ...
    ...Array.from({ length: dagerIMåneden }, (_, i) => {
      ...
      const kanFylle = t.hvem === 'forfatter' && harVært && !d;
      const knapp = el('button', {
        type: 'button',
        text: String(i + 1),
        disabled: !d && !kanFylle,
  ...
  tegn(
    ...
    el('div', { class: 'kort' }, [
      ...
      t.hvem === 'forfatter' && el('p', {
        class: 'liten svak', style: 'margin:.9rem 0 0',
        text: 'Trykk på en tom dag som har vært, så kan du fylle den ut.',
      }),
      valgtDag && dagsark(valgtDag.humor || valgtDag.privat ? valgtDag : null, { egen: t.hvem === 'forfatter' }),
    ]),
 ...
 *
 * Bare tall og forfatterens egne ord. Ingen automatiske tolkninger – de blir fort
  ...
  tegn(
    ...
    el('div', { class: 'kort' }, [
      ...
      el('p', { class: 'midt', style: 'margin:0' }, [
        tall,
        el('span', { class: 'svak', style: 'margin-left:.4rem', text: erForfatter ? 'gode ting skrevet' : 'ting om oss' }),
      ]),
      erForfatter && el('p', {
    ...
    el('div', { class: 'seksjon' }, [
      ...
      el('div', { class: 'brikker' }, [
        el('button', {
          type: 'button', 'aria-pressed': String(!arkivMerket),
          onclick: () => { arkivMerket = false; hentArkiv(); visApp(t); }, text: 'Alt',
        }),
        el('button', {
          type: 'button', 'aria-pressed': String(arkivMerket),
          onclick: () => { arkivMerket = true; hentArkiv(); visApp(t); }, text: '✦ Merket',
        }),
      ]),
      arkivrute,
    ]),
...
/**
 * Spørsmålene forfatterens, som en egen skjerm.
 *
...
async function visQuiz(t, kat = null) {
  ...
  // Ingen kategori valgt: vis dem, med hvor langt forfatteren er kommet i hver.
  if (!kat) {
    ...
    tegn(el('div', { class: 'arsbok' }, [
      ...
      ...bok.maneder.map((m) => el('section', {}, [
        ...
        el('ul', {}, m.ting.map((g) => el('li', {}, [
          ...
          el('span', {}, [g.merket && el('span', { class: 'merke', text: '✦' }), g.tekst]),
        ]))),
      ])),
 ...
 * Delingen.
 *
 * Bryteren sitter hos den det gjelder. Leseren ser hva den står på – det er
 * forskjellen på innsyn og en bakvei – men det er forfatteren som snur den.
 */
function delingKort(t) {
  if (t.hvem !== 'forfatter') {
    return el('div', { class: 'seksjon' }, [
      ...
      el('p', { style: 'margin:.6rem 0 0' }, [
        el('strong', { text: t.delt ? 'Delt modus er på. ' : 'Det velges dag for dag. ' }),
        t.delt
          ? 'Du ser alt forfatteren fører.'
          : 'Du ser dagene forfatteren deler, og at de andre finnes.',
      ]),
      el('p', { class: 'liten svak', style: 'margin:.6rem 0 0', text: 'Den styres av den som skriver.' }),
    ]);
  }
  ...
  return el('div', { class: 'seksjon' }, [
    el('h2', { text: 'Deling' }),
    el('p', { class: 'hjelp', text: 'Hvor mye leseren får se. Du kan snu den når du vil.' }),
    el('div', { class: 'brikker' }, [
      el('button', {
    ...
    el('p', {
      ...
      text: t.delt
        ? 'Alt du fører deles. «Bare for meg» finnes ikke så lenge dette står på.'
        : 'Hver kveld velger du om dagen deles. De du holder for deg selv, ser leseren bare at finnes.',
    }),
  ]);
}
    ...
    el('button', {
      ...
      onclick: async () => {
        try {
          ...
          const lenke = el('a', { href: url, download: `dagbok-${new Date().toISOString().slice(0, 10)}.json` });
 ...
 */
function kodeKort(t) {
  if (!t.kan_bytte_egen && !t.kan_bytte_andres) return null;
  ...
  const bytt = async (hvem, felt) => {
    ...
    try {
      ...
      felt.value = '';
      si(hvem === t.hvem ? 'Koden din er byttet.' : 'Ny kode satt for forfatteren.');
    } catch (e) { si(e.message); }
  };
  ...
  return el('div', { class: 'seksjon' }, [
    ...
    t.kan_bytte_andres && t.hvem !== 'forfatter' && rad('forfatter', 'Ny kode for skriveren', 'Sett'),
    t.kan_bytte_andres && t.hvem !== 'forfatter' && el('p', {
      class: 'liten svak', style: 'margin:.7rem 0 0',
      text: 'Settes den av noen andre, kommer det en melding om det. En kode som byttes i det skjulte, er ikke en kode – det er en lås.',
    }),
  ]);
}
...
async function start() {
  try {
    ...
    // Uten dekning er man ikke logget ut. Å bli kastet til kodefeltet i
...
function visFrakoblet() {
  ...
  // Kommer nettet tilbake av seg selv, skal forfatteren slippe å trykke.
